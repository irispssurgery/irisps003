...
if ($exit_db){
	...
	$query  = " insert into main_board_cfg ". 
			...
			" ,content_color,board_length,pagecount,block,use_html,use_phone,list_reply,list_access ". 
			" ,list_view,list_write,file_attach,file_download,mail_notice,mail_admin,date1 ". 
			" ,ip_admit,memo_admit,admin_id,title_img,skin,color1,color2,color3,category,cautionWord,cautionip ".
			" ) values ( ". 
            " '$table_name','$board_type','$passwd','$boardname','$cool_use','$use_secret','$use_vod','$use_nick' ". 
			" ,$cool_no,'$new_use','$new_time',$title_length,'$title_color' ". 
			" ,'$content_color','$board_length','$pagecount','$block','$use_html','$use_phone','$all_list_reply','$all_list_access' ". 
			" ,'$all_list_view','$all_list_write','$all_file_attach','$all_file_download','$mail_notice', '$mail_admin','$date1','$ip_admit','$memo_admit','$admin_user_id','$title_img','$skin','$color1','$color2','$color3','$category','$cautionWord','$cautionip' )"; 
	$inst->Execute($query);  
