...
*************************************************************/
	...
	if($result) {
		...
		$other01 = $rs["other01"]; //전화번호
		$other02 = $rs["other02"]; //전화번호
		$other03 = $rs["other03"]; //전화번호
		$other04 = $rs["other04"]; //예약날짜
		$other05 = $rs["other05"]; //예약시간
		$other06 = $rs["other06"]; //예약시간
		...
		if($docType=="text") { //텍스트 타입일 경우
			...
			If ($redDate <= 1239116400){ //거제백병원 DB 이전시 태그 문제로 날짝 기준으로 예전 자료는 html 모두 허용
				$useHtml="perfect";		
...
<?endif;?>
		<div class="ViewRow">
			<div class="ViewLabel">예약일</div>
			<div class="ViewDivLine">|</div>
			<div class="ViewRSub"><?=$other04?> <?=$other05?>시 <?=$other06?>분 예약</div>
		</div>
		<div class="ViewRow">
			<div class="ViewLabel">연락처</div>
			<div class="ViewDivLine">|</div>
			<div class="ViewRSub"><?=$other01?>-<?=$other02?>-<?=$other03?></div>
		</div>
		<div class="ViewRow">
...
$btnSms = "<a href=\"javascript:postSms(document.getElementById('__ctl'),'btnSms');\" class=\"lnk_memo\">문자발송</a>";
...
<?
	if($isAdmin || !$useHideButtons) { //비회원,해당회원,관리자일 경우에만 수성/삭제 버튼 보임
			$msg="아이리스성형외과입니다. ".$sms_receive_name."님 예약접수가 성공적으로 되었습니다.";
			$rphone=$other01.$other02.$other03;
?>
			<!-- 문자발송 -->
			<div id="PANEL_COMMENTGROUP">
				<div id="ViewComment_Comment">
					...
					<!-- 코멘트 입력 -->
					<div id="Comment_InputForm" style="">
						<div id="Comment_InputFormTopLayout">
							<div id="Comment_InputFormTop">
								<a href="javascript:formResize(document.getElementById('sms_msg'),2);"><img src="/fsboard/img/clip/dbl_arrow_down.gif" alt="글쓰기 영역 늘리기" /></a><a href="javascript:formResize(document.getElementById('sms_msg'),-2);"><img src="/fsboard/img/clip/dbl_arrow_up.gif" alt="글쓰기 영역 줄이기" /></a>&nbsp;
								<input type="hidden" id="sms_receive_name" name="sms_receive_name" value="<?=$sms_receive_name?>" />
								<input type="hidden" id="rphone" name="rphone" value="<?=$rphone?>" />
								<input type="hidden" id="returnurl" name="returnurl" value="<?=$article_srl?>" />
								<input type="hidden" id="sphone1" name="sphone1" value="051" />
								<input type="hidden" id="sphone2" name="sphone2" value="809" />
								<input type="hidden" id="sphone3" name="sphone3" value="0021" />
								<input type="hidden" id="other07" name="other07" value="yes" />
							</div>
							<div id="btnSms" class="Comment_InputFormBtn">
								<a href="javascript:postSms(document.getElementById('__ctl'),'btnSms');" class="lnk_memo">문자발송</a>&nbsp;&nbsp;
								<input type="radio" id="radio" name="other08" value="접수" checked/>접수&nbsp;&nbsp;
								<input type="radio" id="radio" name="other08" value="대기" />대기
							</div>
						</div>
						<div id="Comment_InputFormMain">
							<div id="Comment_InputFormTextbox"><textarea rows="2" cols="60" id="msg" name="msg" id="msg" style="width:99%; overflow:auto;" class="memo_txtbox2"><?=$msg?></textarea></div>
						</div>
					</div>
...
<div id="ViewBottomBtn">
	...
	<div id="ViewBottomBtnLeft"><?echo "{$btnEdit} {$btnRemove} ";?></div>
...
function postSms(frm,btnId) { //메모글 전송
	var chk = smsSend(frm);

	if(chk) {
		document.getElementById(btnId).style.visibility = "hidden";
		<?echo "sendit('{$id}','sendSMS',{$idx},{$page},'{$srhctgr}','".str_replace("'","&#39;",$srhstr)."','{$rowctgr}','$rowmode','".str_replace("'","&#39;",$ctgrstr)."');";?>
