...
*************************************************************/
	...
	$rowSms = "<a href=\"".$_SERVER["PHP_SELF"]."?id={$id}&amp;mode=list&amp;srhctgr={$srhctgr}&amp;srhstr={$srhstr}&amp;page={$page}&amp;rowctgr=other07&amp;rowmode={$rowmode}\" class=\"lnk_list\" title=\"조회수 오름차순/내림차순 정렬\">";
	$rowReserve = "<a href=\"".$_SERVER["PHP_SELF"]."?id={$id}&amp;mode=list&amp;srhctgr={$srhctgr}&amp;srhstr={$srhstr}&amp;page={$page}&amp;rowctgr=other08&amp;rowmode={$rowmode}\" class=\"lnk_list\" title=\"조회수 오름차순/내림차순 정렬\">";
...
if(!$view_included) { ContentTop(); }
	...
	<div id=\"ListFrame\">
		<div id=\"ListLayout\">
			...
			<div id=\"ListTitle\">
				...
				<h4 class=\"ListBody7\">".$rowSms."SMS</a></h4>
				<h4 class=\"ListBody8\">".$rowReserve."예약</a></h4>
			</div>
	";
	...
	if($allowNoticeNum>0 && $noticeNum>0 && $page==1 && !intval($srhctgr) && !$srhstr && !$ctgrstr) {
		...
		while($nrs = mysql_fetch_array($nresult)) {
			...
			$other07 = Trim($nrs["other07"]);
			$other08 = Trim($nrs["other08"]);
			...
			<div class=\"ListBody\">
				...
				<p class=\"ListBody7\">".$sms_str."</p>
				<p class=\"ListBody8\">".$other08."</p>
			</div>
	...
	if($result=mysql_query($query)) {
		while($rs=mysql_fetch_array($result)) {
			...
			$other07 = Trim($rs["other07"]);
			$other08 =  Trim($rs["other08"]);
			...
			//SMS
			if($other07=="yes") {
				$sms_str = "<font color='#DB6B2A'><b>발송</b></font>";
			}else{
				$sms_str = "<font color='#9704FF'><b>미발송</b></font>";
			}

			//상태
			if ($other08=="접수"){
				$other08_str="<font color='#3A6F83'><b>접수</b></font>";
			}elseif ($other08=="대기"){
				$other08_str="<font color='#589054'><b>대기</b></font>";				
			}
			...
			<div class=\"ListBody\">
				...
				<p class=\"ListBody6\">{$reg_date}</p>
				<p class=\"ListBody7\">{$sms_str}</p>
				<p class=\"ListBody8\">{$other08_str}</p>
			</div>
			";
			$sequenceNum--;
			$i++;
