...
*************************************************************/
	...
	$totalPage = intval(($totalObj - 1) / $pageSize) + 1; //총 페이지수
	...
	if(!$MemId) {
		$btnLogin = "<a href=\"".$_SERVER["PHP_SELF"]."?".QryStr("login")."&page={$page}\" title=\"로그인\"><span style=\"font-size:11px;font-family:Arial;\" class=\"lnk_list\">Login</span></a>";
		...
		$btnLogout = "<a href=\"".$_SERVER["PHP_SELF"]."?".QryStr("logout")."&page={$page}\" title=\"로그아웃\"><span style=\"font-size:11px;font-family:Arial;\" class=\"lnk_list\">Logout</span></a>";
	...
	if($isAdmin || !$useHideButtons) {
		...
		$btnAdmin = "<a href=\"".$_SERVER["PHP_SELF"]."?".QryStr("admin")."&page={$page}\" title=\"게시판 관리\"><span style=\"font-size:11px;font-family:Arial;\" class=\"lnk_list\">Admin</span></a>";
	...
	if($MemLevel<=1) $btnSetup = "<a href=\"".$_SERVER["PHP_SELF"]."?".QryStr("setup")."&page={$page}\" onclick=\"window.open(this.href,'_blank'); return false;\" title=\"관리자 설정\"><span style=\"font-size:11px;font-family:Arial;\" class=\"lnk_list\">Setup</span></a>";

	if($write_level || !$useHideButtons) $btnWrite = "<a href=\"".$_SERVER["PHP_SELF"]."?".QryStr("write")."&page={$page}\" class=\"list_btn\"><img src=\"{$btnPath}/write.gif\" alt=\"글쓰기\" /></a>";

	$btnSrh = "<a href=\"javascript:search();\"><img src=\"{$FSBOARD_PATH}/img/clip/search.gif\" alt=\"검색\" style=\"vertical-align:middle;\" /></a>";
	$btnSrhList = $srhctgr&&$srhstr||$mode=="search"||$rowctgr&&$rowmode||$ctgrstr ? "<a href=\"".$_SERVER["PHP_SELF"]."?id={$id}&mode=list\"><img src=\"{$FSBOARD_PATH}/img/btn/list.gif\" alt=\"검색전 목록으로\" style=\"vertical-align:middle;\" /></a>" : "";
	$btnList = $mode=="view" ? "<a href=\"".$_SERVER["PHP_SELF"]."?id={$id}&mode=list\" class=\"lnk_list\"><img src=\"{$btnPath}/list.gif\" alt=\"리스트\" /></a>" : "";

	if($useRssFeed) $btnRss = "<a href=\"".$_SERVER["PHP_SELF"]."?id={$id}&mode=rss.xml\"><img src=\"{$FSBOARD_PATH}/img/clip/rss.gif\" alt=\"RSS 2.0 Feeds(XML)\" /></a>";
...
if(!$view_included) { ContentTop(); }
	...
	if($allowNoticeNum>0 && $noticeNum>0 && $page==1 && !intval($srhctgr) && !$srhstr && !$ctgrstr) {
		...
		while($nrs = mysql_fetch_array($nresult)) {
			...
			$subject = "<b><a href=\"".$_SERVER["PHP_SELF"]."?".QryStr("view")."&idx={$idx}&page={$page}\" title=\"{$contents}\" class=\"lnk_list\">{$subject}</a></b>";

			//댓글 갯수 표시
			if($memoNum) {
				$iconMemoNum = "<span class=\"list_m\">[$memoNum]</span>";
				if((mktime() - $nrs["memoLatestDate"]) <= $nterm) $iconMemoNum = "<b>{$iconMemoNum}</b>";
			}
			else $iconMemoNum = "";
			...
			//첨부파일이 있을 경우 표시
			$iconFile = $fileName1 ? "<img src=\"{$FSBOARD_PATH}/img/clip/file.gif\" alt=\"첨부파일\" />" : "";

			//작성자 표시
			$author = MemberName($isMember,$author,$authorLimit,$e_mail,$homeUrl);

			//새글에 new표시
			$icoNew = ((mktime() - $regDate) <= $nterm) ? "<img src=\"{$FSBOARD_PATH}/img/clip/new.gif\" alt=\"최근글\" />" : "";
			...
			<div class=\"ListBody\">
				...
				<p class=\"ListBody2\">".($idx==$view_idx?"<img src=\"{$FSBOARD_PATH}/img/clip/arrow3.gif\" alt=\"현재글\" style=\"vertical-align:baseline;\" />":"<img src=\"{$FSBOARD_PATH}/img/clip/notice.gif\" alt=\"공지글\" style=\"vertical-align:baseline;\" />")."</p>
	...
	if($result=mysql_query($query)) {
		...
		while($rs=mysql_fetch_array($result)) {
			...
			//작성자 표시
			$author_origin = StripHtmlChars($author);
			$author = MemberName($isMember,$author,$authorLimit,$e_mail,$homeUrl);

			//답글일 경우 표시
			if($useReply) $iconRe = $reLevel ? "<img src=\"{$FSBOARD_PATH}/img/clip/re_arrow.gif\" alt=\"답글\" style=\"margin-left:".($reLevel*7)."px;vertical-align:middle;\" /> " : "";

			//view링크
			$viewlink = "<a href=\"".$_SERVER["PHP_SELF"]."?".QryStr("view")."&idx={$idx}&page={$page}\" title=\"{$contents}\" class=\"lnk_list\">";
			...
			if(substr($srhctgr,2,1)&&$srhstr) {
				if($srhstring) {
					for($j=0,$till=sizeof($srhstring); $j<$till; $j++)
						$subject = str_replace($srhstring[$j],"<b style=\"background-color:yellow;\">".$srhstring[$j]."</b>",$subject);
				}
				else $subject = str_replace($srhstr,"<b style=\"background-color:yellow;\">{$srhstr}</b>",$subject);
			}
			$subject = "{$viewlink}{$subject}</a>";
			$subject = "{$iconRe}{$subject}";

			//댓글갯수 표시
			if($memoNum||$tbNum) {
				$iconMemoNum = "<span class=\"list_m\">(".($memoNum+$tbNum).")</span>";
				if((mktime() - $rs["memoLatestDate"]) <= $nterm) $iconMemoNum = "<b>{$iconMemoNum}</b>";
			}
			else $iconMemoNum = "";
			...
			if($isSecret) {
				$prvImg = eregi("\.jpg|\.gif|\.bmp|\.png",$fileName1)&&$isAdmin ? "{$PHP_SELF}?id={$id}&mode=filelink&maintainCode={$maintainCode}&filename=".urlencode($fileName1) : "$FSBOARD_PATH/img/clip/no_image.gif";
			}
			else {
				$prvImg = eregi("\.jpg|\.gif|\.bmp|\.png",$fileName1) ? "{$PHP_SELF}?id={$id}&mode=filelink&maintainCode={$maintainCode}&filename=".urlencode($fileName1) : "$FSBOARD_PATH/img/clip/no_image.gif";
			}
			...
			echo "
							<td class=\"ListPicCell\">
								<table style=\"width:".($iwidth+10)."px; word-break:break-all; border:1px solid #e1e1e1;\">
									<tr>
										<td>{$viewlink}<img src=\"{$prvImg}\" ".($idx==$view_idx?" class=\"imgover\"":" class=\"imgbase\" onmouseout=\"this.className='imgbase'\"")." onmouseover=\"this.className='imgover';\" alt=\"작성자: {$author_origin}\n등록일: {$reg_date}\n조회수: {$readNum}\" style=\"width:{$iwidth}px;height:{$iheight}px;\" /></a></td>
									</tr>
									<tr>
										<td class=\"list_lnk\">".
											($isAdmin?"<input type=\"checkbox\" name=\"idx{$i}\" value=\"{$idx}\" />":"").
											"{$subject}{$iconLock}{$icoNew}{$iconMemoNum}
											<!--
											<div>{$author}</div>
											<div class=\"list_d\">{$reg_date}</div>
											<div class=\"list_r\">Read:{$readNum}</div>
											-->
										</td>
									</tr>
								</table>
			...
			";
			if(($i+1-($page>1?0:$noticeNum))%4 == 0) {
				echo "
						</tr>
						<tr>
							<td class=\"ListPicDiv\"></td>
						</tr>
						<tr>
				";
			}
		...
		if(!$i) {
			...
			<div class=\"ListBody\"><p style=\"text-align:center\">\"<b>{$srhstr}</b>\" (으)로 검색된 결과가 없습니다</p></div>
				...
				echo "
			<div class=\"ListBody\"><p style=\"text-align:center\">등록된 사진이 없습니다</p></div>
				";
			}
		}
...
?>
			...
			<div id="ListBottom">
				<!-- 페이징 --><?=NavPage($page,$divPage,$totalPage,QryStr("list")," class=\"list_p\"","◀,▶"," , ")?>
			</div>
...
function category_change(obj) {
	try {
		...
		window.location.href = "<?echo $_SERVER["PHP_SELF"]."?id={$id}&amp;mode={$list}&amp;ctgrstr=\"+ctgrstr+\"&amp;page=1";?>";
		return true;
	} catch(e) {}
}
//]]>
