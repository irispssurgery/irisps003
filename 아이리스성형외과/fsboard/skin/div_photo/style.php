...
input.txtbox_srh { border:0 solid #e1e1e1; height:15px; }
	...
	/* 포토게시판 리스트 스타일 */
	.imgbase { border:1px #E0E0E0 solid; }
	.imgover {
		/*
		...
		*/
		filter:gray(enabled=1); border:1px #000000 solid;
	}
...
/* 레이아웃 공통 */
div#ListFrame, div#ViewFrame, div#WriteFrame { clear:both; margin:0 auto; text-align:<?=$align?>; background-color:<?=$bgColor?>; <?=($bgImage?"background:url({$bgImage}); ":"")?>font-size:12px; font-family:맑은 고딕, 돋움, Verdana; white-space:nowrap; }
...
/* 리스트 - 레이아웃 스타일 */
	#ListTop { width: 100%; margin:0; } #ListTop p { margin:0; }
	...
	#ListSearch { clear:both; width:<?=(($srhctgr&&$srhstr)||$ctgrstr||($rowctgr&&$rowmode)?"32.1":"27.9")?>em; height:1.7em; margin:0 auto; }
		#ListSearch p { float:left; margin:0; height:1.5em; }
		p.list_srhtxt { border:1px solid #e1e1e1; }

	/* 포토게시판 레이아웃 스타일 */
	#ListPicTable { width:100%; margin:10px 0 0 0; }
		.ListPicCell { width:20%; text-align:center; vertical-align:top; }
